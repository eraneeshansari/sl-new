	  ...
	  stage('Deploy to k8s'){
	    steps{
		  ...
		  sshagent(['kops-machine']){
		    sh "scp -o StrictHostKeyChecking=no pod.yaml ubuntu@13.235.50.169:/home/ubuntu/"
			script{
			    try{
				  sh "ssh ubuntu@13.235.50.169 kubectl apply -f ."				 
				}
				catch(error){
				  sh "ssh ubuntu@13.235.50.169 kubectl create -f ."
				}
			}
